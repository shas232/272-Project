...
export default function RiskInsights() {
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      try {
        const response = await fetch("http://54.162.232.80:5008/api/getMonthlyExpenseStats");
    ...
    const fetchFraudData = async () => {
      try {
        const response = await fetch("http://54.162.232.80:5008/api/getMonthlyFraudStats");
        const data = await response.json();
